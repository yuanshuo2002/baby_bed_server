...
from app.models.device import VALID_WORK_MODES
...
class HardwareService:
    ...
    async def switch_device_mode(device_sn: str, mode: str, baby_id: int | None = None) -> dict:
        ...
        Args:
            mode: 工作模式，可选值: sleep(睡眠), play(游戏), co_sleep(拼床)
        """
        if mode not in VALID_WORK_MODES:
            raise ValueError(f"无效的工作模式: {mode}, 可选值: {list(VALID_WORK_MODES)}")
        ...
        config = mode_configs.get(mode, mode_configs["sleep"])
    ...
    async def get_device_mode(device_sn: str) -> dict:
        ...
        return {
            "device_sn": device_sn,
            "current_mode": "sleep",
            "available_modes": list(VALID_WORK_MODES),
